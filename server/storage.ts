...
export class DatabaseStorage implements IStorage {
  ...
  // HR System - Timbrature
  async getTimbrature(personaleId?: string): Promise<Timbratura[]> {
    if (personaleId) {
      return await db.select().from(timbrature).where(eq(timbrature.personaleId, personaleId)).orderBy(desc(timbrature.dataOra));
    }
    return await db.select().from(timbrature).orderBy(desc(timbrature.dataOra));
  }

  async getTimbratura(id: string): Promise<Timbratura | undefined> {
    const [result] = await db.select().from(timbrature).where(eq(timbrature.id, id));
    return result;
  }

  async createTimbratura(data: InsertTimbratura): Promise<Timbratura> {
    const id = randomUUID();
    const [result] = await db.insert(timbrature).values({ ...data, id, createdAt: new Date().toISOString() }).returning();
    return result;
  }

  async updateTimbratura(id: string, data: Partial<InsertTimbratura>): Promise<Timbratura | undefined> {
    const [result] = await db.update(timbrature).set(data).where(eq(timbrature.id, id)).returning();
    return result;
  }
  ...
  // HR System - Turni
  async getTurni(personaleId?: string): Promise<Turno[]> {
    if (personaleId) {
      return await db.select().from(turni).where(eq(turni.personaleId, personaleId)).orderBy(desc(turni.data));
    }
    return await db.select().from(turni).orderBy(desc(turni.data));
  }
  ...
  async createTurno(data: InsertTurno): Promise<Turno> {
    const id = randomUUID();
    const [result] = await db.insert(turni).values({ ...data, id, createdAt: new Date().toISOString() }).returning();
    return result;
  }

  async updateTurno(id: string, data: Partial<InsertTurno>): Promise<Turno | undefined> {
    const [result] = await db.update(turni).set(data).where(eq(turni.id, id)).returning();
    return result;
  }
  ...
  // HR System - Richieste Assenza
  async getRichiesteAssenza(personaleId?: string): Promise<RichiestaAssenza[]> {
    if (personaleId) {
      return await db.select().from(richiesteAssenza).where(eq(richiesteAssenza.personaleId, personaleId)).orderBy(desc(richiesteAssenza.createdAt));
    }
    return await db.select().from(richiesteAssenza).orderBy(desc(richiesteAssenza.createdAt));
  }

  async getRichiestaAssenza(id: string): Promise<RichiestaAssenza | undefined> {
    const [result] = await db.select().from(richiesteAssenza).where(eq(richiesteAssenza.id, id));
    return result;
  }

  async createRichiestaAssenza(data: InsertRichiestaAssenza): Promise<RichiestaAssenza> {
    const id = randomUUID();
    const [result] = await db.insert(richiesteAssenza).values({ ...data, id, createdAt: new Date().toISOString() }).returning();
    return result;
  }

  async updateRichiestaAssenza(id: string, data: Partial<InsertRichiestaAssenza>): Promise<RichiestaAssenza | undefined> {
    const [result] = await db.update(richiesteAssenza).set(data).where(eq(richiesteAssenza.id, id)).returning();
    return result;
  }
